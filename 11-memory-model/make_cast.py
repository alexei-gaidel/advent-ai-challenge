"""Собирает demo.svg — анимированную запись живого прогона модели памяти.
...
SETTINGS = {
    ...
    "max_tokens": 120,
    "keep_last": 6,
    "system_prompt": "Ты ассистент команды. Отвечай одной короткой фразой.",
}
...
class Cast:
    ...
    def svg(self):
        ...
        parts.append(f'<circle cx="20" cy="16" r="5" fill="#ef4444"/>'
                     ...
                     f'python3 make_cast.py — слои памяти агента</text>')
...
def main():
    ...
    cast.say("$ python3 web.py   # модель памяти: три слоя", "dim")
    cast.say(f"агент {agent.id}: краткосрочная · рабочая · долговременная", "head")
    cast.say()

    for text in ("Меня зовут Алекс, я тимлид из Казани.",
                 "Бюджет задачи 2,5 млн, срок до 1 декабря."):
        cast.say(f"вы → {text}", "user")
        result = agent.ask(text)
        cast.say(f"агент ← {result['text'].strip().splitlines()[0][:78]}", "agent")
        for proposal in result["proposed"]:
            agent.decide(proposal["id"], "accept")
            layer = memory.LAYERS[proposal["layer"]]["title"].lower()
            cast.say(f"  роутер → {layer}: {proposal['key']} = {proposal['value'][:40]}", "warn")
        cast.say()

    snapshot = agent.snapshot()
    cast.say(f"рабочая: {len(snapshot['layers']['working']['items'])} записей · "
             f"долговременная: {len(snapshot['layers']['long']['items'])} записей", "head")
    cast.say()

    question = "Как меня зовут и какой у нас бюджет? Одной строкой."
    agent.update({"use_short": False, "router": False})
    cast.say("Краткосрочную память выключаем — отвечать можно только из слоёв.", "dim")
    cast.say(f"вы → {question}", "user")
    cast.say(f"агент ← {agent.ask(question)['text'].strip()[:78]}", "good")
    cast.say()

    agent.update({"use_long": False})
    cast.say("Выключаем долговременную память…", "warn")
    cast.say(f"вы → {question}", "user")
    cast.say(f"агент ← {agent.ask(question)['text'].strip()[:78]}", "agent")
    cast.say()

    agent.update({"use_long": True, "use_working": False})
    cast.say("Возвращаем долговременную, выключаем рабочую…", "warn")
    cast.say(f"вы → {question}", "user")
    cast.say(f"агент ← {agent.ask(question)['text'].strip()[:78]}", "agent")
    cast.say()
    cast.say("каждый слой отвечает за свою часть ответа", "good")

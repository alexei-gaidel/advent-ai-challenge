...
DEFAULTS = {
    ...
    # Инструменты MCP: давать ли модели доступ к git-серверу и сколько кругов вызовов.
    "use_tools": True,
    "max_tool_rounds": 3,
...
EMPTY_STATS = {"calls": 0, "prompt_tokens": 0, "completion_tokens": 0, "cost": 0.0,
               ...
               # Вызовы инструментов MCP и круги, за которые модель дошла до ответа.
               "tool_calls": 0, "tool_rounds": 0}
...
class Agent:
    ...
    def ask(self, text):
        ...
        result, used_tools = self._run_with_tools(messages)
        ...
        # Токены и стоимость уже посчитаны в _run_with_tools по всем кругам.
        self.stats["calls"] += 1
        ...
        return {**result, "agent": self.snapshot(), "data_savings": data_savings,
                ...
                "audit": audit, "tool_calls": used_tools}

    def _run_with_tools(self, messages):
        """Запрос к модели с доступом к инструментам MCP.

        Модель может ответить не текстом, а списком вызовов: тогда выполняем их
        через MCP, кладём результаты в диалог и спрашиваем снова — пока не получим
        обычный ответ или не кончатся круги.
        """
        specs = mcp_tools.BRIDGE.specs() if self.settings["use_tools"] else []
        ...
        for round_number in range(rounds + 1):
            ...
            result = providers.call(
                ...
                tools=None if (last_round or not specs) else specs,
            )
            ...
            for call in result["tool_calls"]:
                ...
                arguments = mcp_tools.parse_arguments(function.get("arguments"))
                output = mcp_tools.BRIDGE.call(name, arguments)
                self.stats["tool_calls"] += 1
                used.append({"name": name, "arguments": arguments,
                             "result": output[:600], "round": round_number + 1})
                conversation.append({
                    "role": "tool",

...
-- Сводка хранится отдельно от сообщений: сами реплики остаются в базе целиком,
-- а в запрос к модели вместо них уходит summary.
...
def save_summary(agent_id, text, covered):
    """Сохраняет сводку агента: сам текст и сколько сообщений она покрывает."""
    with connect() as connection:
        connection.execute(
            """INSERT INTO summaries (agent_id, text, covered, updated_at)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(agent_id) DO UPDATE SET
                   text = excluded.text,
                   covered = excluded.covered,
                   updated_at = excluded.updated_at""",
            (agent_id, text, covered, now()),
        )


def clear_summary(agent_id):
    with connect() as connection:
        connection.execute("DELETE FROM summaries WHERE agent_id = ?", (agent_id,))


def load_all():
    """Все агенты с их историей и сводкой — этим наполняется реестр при старте."""
    with connect() as connection:
        ...
        summaries = {
            row["agent_id"]: {"text": row["text"], "covered": row["covered"]}
            for row in connection.execute("SELECT agent_id, text, covered FROM summaries")
        }
    ...
    return [
        {
            ...
            "summary": summaries.get(agent["id"], {"text": "", "covered": 0}),
        }
        for agent in agents
    ]
...
def stats():
    ...
    with connect() as connection:
        ...
        summaries = connection.execute("SELECT COUNT(*) AS n FROM summaries").fetchone()["n"]
    return {"agents": agents, "messages": messages, "summaries": summaries,
            "path": str(DB_PATH)}

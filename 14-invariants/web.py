  ...
  function showTask(state, snapshot) {
    ...
    box.querySelectorAll(".stages button").forEach((button) => {
      button.onclick = async () => {
        ...
        if (res.error) {
          // Автомат отказал — показываем причину и предлагаем перейти принудительно.
          if (confirm(res.text + "\n\nПерейти всё равно?")) {
            applySnapshot(state, await api(`/api/agents/${state.id}/task/move`,
              { stage: button.dataset.k, force: true }), false);
          }
          return;
        }
        ...
        try:
            ...
            # --- профили: несколько штук, живут отдельно от агентов ---
                ...
                if action == "start":
                    ...
                    self._json(agent.move(payload["stage"], bool(payload.get("force"))))
                elif action == "pause":
                    ...
                    if payload.get("accept"):
                        self._json(agent.apply_advice(force=bool(payload.get("force"))))
                    else:

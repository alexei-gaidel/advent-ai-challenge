...
Запуск:
    python3 web.py     → откроется http://localhost:8019
...
<title>Пайплайн: билеты на матч</title>
<style>
...
<div class="wrap">
  ...
  <div class="market" id="pipeline"><h2>Загружаю состояние…</h2></div>
...
<script>
  ...
  async function refreshPipeline() {
    ...
    try {
      ...
      box.innerHTML = "<h2>Билеты</h2><div>не удалось получить данные: " + esc(err) + "</div>";
      return;
    }

    const changedReports = (data.reports || []).filter((r) => r.changed);
    box.innerHTML = "<h2>" + esc(data.event.title) +
      '<button class="ghost" id="runnow" style="font-size:12px;padding:5px 10px">Проверить сейчас</button>' +
      '<span style="flex:1"></span><span style="color:var(--muted);font-size:12.5px">' +
      "снимков " + data.snapshots + "</span></h2>" +
      '<div class="cols"><div><pre>' + esc(data.described) + "</pre>" +
      (data.snapshot && data.snapshot.offers ? "<table>" + data.snapshot.offers.map((o) =>
        "<tr><td>" + esc(o.min_price.toLocaleString("ru-RU")) + " ₽</td><td>" +
        esc(o.max_price.toLocaleString("ru-RU")) + " ₽</td><td class='" +
        (o.sale_status === "available" ? "ok" : "bad") + "'>" + esc(o.sale_status) +
        "</td></tr>").join("") + "</table>" : "") +
      '<div style="margin-top:8px"><a href="' + esc(data.event.url) +
      '" target="_blank" style="color:var(--accent);font-size:12.5px">страница события</a></div>' +
      "</div><div>" +
      "<b style='font-size:13px'>Расписание</b><table>" + (data.jobs || []).map((job) =>
        "<tr><td>" + esc(job.name) + "</td><td>каждый час в :" +
        String((job.params && job.params.at_minute) || 0).padStart(2, "0") +
        "</td><td>" + esc(job.next_run.slice(11, 19)) + "</td></tr>").join("") + "</table>" +
      "<b style='font-size:13px'>Отчёты с изменениями</b><table>" +
      (changedReports.length ? changedReports.map((r) =>
        "<tr><td>" + esc(r.created_at.slice(5, 16).replace("T", " ")) + "</td><td>" +
        esc(r.summary.split("\n").filter((l) => l.startsWith("•"))[0] || "изменения") +
        "</td></tr>").join("") : "<tr><td colspan=2>пока не было</td></tr>") + "</table>" +
      "<b style='font-size:13px'>Последние прогоны</b><table>" +
      (data.runs || []).map((run) =>
        "<tr><td>" + esc(run.at.slice(11, 19)) + "</td><td class='" +
        (run.status === "ok" ? "ok" : "bad") + "'>" + esc(run.status) + "</td><td>" +
        esc(run.detail.slice(0, 44)) + "</td></tr>").join("") +
      "</table></div></div>";
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _pipeline_state(self):
        ...
        return {
            "event": {"title": tickets.EVENT_TITLE, "url": tickets.EVENT_URL},
            "snapshot": last,
            "described": tickets.describe(last) if last else "снимков пока нет",
            "snapshots": len(snapshots),
            "reports": store.list_reports(6),
    ...
    def do_GET(self):
        if self.path == "/api/pipeline":
            self._json(self._pipeline_state())
            return
    ...
    def do_POST(self):
        if self.path == "/api/pipeline/run":
            try:
                result = pipeline.run(notify_on_change=False, verbose=False)
                self._json({"ok": True, "detail": result["detail"]})

...
def call(model_id, messages, temperature=0.7, max_tokens=800, stop=None, tools=None):
    ...
    if stop:
        body["stop"] = stop
    if tools:
        # Описания инструментов в формате function calling: модель сама решает,
        # вызывать ли что-то, и возвращает tool_calls вместо обычного ответа.
        body["tools"] = tools
    ...
    return {
        # При вызове инструмента content пустой — это не «пустой ответ», а ход модели.
        "text": (strip_thinking(message.get("content") or "")
                 or ("" if tool_calls else "(пустой ответ)")),

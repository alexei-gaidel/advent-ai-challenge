"""Веб-чат с агентами и тремя стратегиями управления контекстом.
...
<title>Стратегии управления контекстом</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .factsbox { font-size:12.5px; }
  .factstable { width:100%; border-collapse:collapse; margin-top:7px; font-size:12px; }
  .factstable td { padding:3px 6px; border-bottom:1px solid var(--border);
                   vertical-align:top; }
  .factstable td.k { color:var(--muted); white-space:nowrap; width:1%; }
  .branchbar { display:flex; flex-wrap:wrap; gap:5px; padding:9px 14px;
               border-bottom:1px solid var(--border); }
  .branchbar button { font-size:12px; padding:5px 10px; background:var(--bg);
                      color:var(--muted); border:1px solid var(--border); }
  .branchbar button.on { background:var(--accent); color:#fff; border-color:var(--accent); }
...
<div class="wrap">
  ...
  <div class="sub" id="dbline">Каждая панель — отдельный агент на сервере; Sliding Window, Sticky Facts и Branching — переключаются на лету</div>
...
<script>
  ...
  function statsLine(snapshot) {
    ...
    return "вызовов: <b>" + s.calls + "</b> · токенов: <b>" +
      ...
      "</b><br>стратегия: <b>" + STRATEGIES[snapshot.settings.strategy].title +
      "</b> · в ветке: <b>" + snapshot.messages + "</b> сообщ. · уходит в модель: <b>" +
      snapshot.in_context + "</b>" +
      (s.facts_calls ? " · обновлений фактов: <b>" + s.facts_calls + "</b> на " +
        s.facts_tokens + " токенов" : "");
  }

  function showFacts(state, snapshot) {
    const box = state.panel.querySelector(".factsbox");
    const keys = Object.keys(snapshot.facts || {});
    if (!keys.length) { box.style.display = "none"; return; }
    box.style.display = "block";
    state.panel.querySelector(".factstable").innerHTML = keys
      .map((k) => '<tr><td class="k">' + esc(k) + "</td><td>" + esc(snapshot.facts[k]) + "</td></tr>")
      .join("");
  }

  function showBranches(state, snapshot) {
    const bar = state.panel.querySelector(".branchbar");
    if (snapshot.branches.length < 2) { bar.style.display = "none"; return; }
    bar.style.display = "flex";
    bar.innerHTML = snapshot.branches
      .map((b) => '<button data-b="' + b.id + '"' +
        (b.id === snapshot.branch ? ' class="on"' : "") + ">" + esc(b.title) +
        " · " + b.messages + "</button>").join("");
    bar.querySelectorAll("button").forEach((button) => {
      button.onclick = async () => {
        const snap = await api(`/api/agents/${state.id}/switch`, { branch: button.dataset.b });
  ...
  function applySnapshot(state, snapshot, repaintFeed) {
    ...
    showFacts(state, snapshot);
    showBranches(state, snapshot);
    paintStrategy(state);
    if (repaintFeed) {
  ...
  function paintStrategy(state) {
    state.panel.querySelectorAll(".strategy").forEach((b) =>
      b.classList.toggle("on", b.dataset.s === state.settings.strategy));
    // Окно не участвует в ветвлении — гасим поле, чтобы не вводило в заблуждение.
    const windowed = state.settings.strategy !== "branching";
    state.panel.querySelector(".keep").disabled = !windowed;
  }
  ...
  function build(snapshot) {
    ...
    panel.innerHTML = `
      ...
      <details class="cfg">
        ...
        <div class="body">
          ...
          <div class="row"><label>Стратегия</label>
            <div class="seg strat">
              ${Object.entries(STRATEGIES).map(([k, v]) =>
                `<button class="strategy" data-s="${k}" title="${v.note}">${v.title}</button>`).join("")}
            </div></div>
          <div class="row"><label>Память</label>
            <input type="checkbox" class="mem" ${st.memory ? "checked" : ""}>
            <span style="color:var(--muted);font-size:12.5px">окно: последние</span>
          ...
          <details class="factsbox" style="display:none"><summary>Факты (ключ-значение)</summary>
            <table class="factstable"></table>
            <button class="ghost clearfacts" style="font-size:12px;margin-top:7px">Очистить факты</button>
          </details>
      ...
      <div class="branchbar" style="display:none"></div>
      <div class="feed"></div>
      <div class="tools">
        <button class="ghost reset">Сбросить диалог</button>
        <button class="ghost cloneme">Клонировать</button>
        <button class="ghost cp">Чекпоинт</button>
        <button class="ghost forkme">Ветка отсюда</button>
      </div>
      <div class="ask">
    ...
    panel.querySelectorAll(".strategy").forEach((button) => {
      button.onclick = async () => {
        const snap = await push({ strategy: button.dataset.s });
        applySnapshot(state, snap, false);
      };
    });
    q(".clearfacts").onclick = async () =>
      applySnapshot(state, await api(`/api/agents/${state.id}/facts/clear`), false);
    q(".cp").onclick = async () => {
      const snap = await api(`/api/agents/${state.id}/checkpoint`, {});
      const feed = q(".feed");
      feed.insertAdjacentHTML("beforeend",
        '<div class="meta" style="text-align:center">— чекпоинт: ' +
        esc(snap.checkpoints[snap.checkpoints.length - 1].title) + " —</div>");
      applySnapshot(state, snap, false);
    };
  ...
  async function ask(id, text) {
    ...
    if (data.facts_update) {
      pending.insertAdjacentHTML("beforebegin",
        '<div class="meta" style="text-align:center">— карточка фактов обновлена: ' +
        data.facts_update.keys + " ключей, " + data.facts_update.tokens + " токенов —</div>");
    }
    feed.scrollTop = feed.scrollHeight;
  }
...
def build_page():
    ...
    html = html.replace("__STRATEGIES__", json.dumps(STRATEGIES, ensure_ascii=False))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/agents":
            ...
            elif len(parts) == 5 and parts[:2] == ["api", "agents"] and parts[3] == "facts":
                agent = agents.get(parts[2])
                agent.facts = {}
                storage.clear_facts(agent.id)
                ...
                if action == "ask":
                    ...
                    self._json(agent.checkpoint(payload.get("title", "")))
                elif action == "fork":
                    self._json(agent.fork(payload.get("checkpoint"), payload.get("title", "")))
                elif action == "switch":
                    self._json(agent.switch(payload["branch"]))
                elif action == "close":

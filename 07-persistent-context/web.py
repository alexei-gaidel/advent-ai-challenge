...
<title>Агенты с постоянной памятью</title>
<style>
...
<div class="wrap">
  ...
  <div class="sub" id="dbline">Каждая панель — отдельный агент на сервере; история пишется в SQLite и переживает перезапуск</div>
...
<script>
  ...
  (async () => {
    ...
    if (state.agents.length) {
      ...
      for (const snapshot of state.agents) paintHistory(build(snapshot), snapshot.history);

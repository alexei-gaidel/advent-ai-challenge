...
class Agent:
    ...
    def move(self, target, force=False):
        self._require_task()
        taskstate.transition(self.task, target, force)
        self.task_proposal = None
        self._save_task()
        return self.snapshot()
    ...
    def apply_advice(self, accept_step=True, accept_stage=True, accept_expected=True,
        ...
        if accept_stage and advice["next_stage"]:
            taskstate.transition(self.task, advice["next_stage"], force)
    ...
    def _messages(self, question):
        ...
        return messages, {**layers["sizes"], "task": len(state)}
